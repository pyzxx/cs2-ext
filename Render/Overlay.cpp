...
void Overlay::Render()
{
	...
	ImGui::Begin("external cs2 cheat", &RenderMenu, ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoScrollbar);
	...
	if (ImGui::BeginTabBar("Tabs"))
	{
		...
		{
			ImGui::Checkbox("activate aimbot", &g.aimbot);
			ImGui::Checkbox("Draw FOV", &g.showFov);
			ImGui::SliderFloat("FOV", &g.Aimfov, 30.f, 200.f);
			ImGui::SliderFloat("smoothing", &g.Aimsmoothing, 0.2f, 3.f);
			ImGui::EndTabItem();
		}
		...
		{
			ImGui::SliderInt("Screen FOV", &g.screenfov, 0.0f, 170.0f); // Mudado para float

			// Nome do jogador
			ImGui::Checkbox("name", &g.name); // Checkbox para mostrar o nome do jogador
			...
			if (g.name)
			{
				...
				ImGui::SameLine();
				if (ImGui::ColorButton("##NameColor", g.nameColor))
					ImGui::OpenPopup("Select Name Color");
				if (ImGui::BeginPopup("Select Name Color"))
				{
					ImGui::ColorEdit4("Name Color", (float*)&g.nameColor, ImGuiColorEditFlags_NoInputs | ImGuiColorEditFlags_NoLabel);
					ImGui::EndPopup();
				}
			}
			...
			if (g.e_box)
			{
				...
				ImGui::SameLine();
				if (ImGui::ColorButton("##BoxColor", g.boxColor))
					ImGui::OpenPopup("Select Box Color");
				if (ImGui::BeginPopup("Select Box Color"))
				{
					ImGui::ColorEdit4("Box Color", (float*)&g.boxColor, ImGuiColorEditFlags_NoInputs | ImGuiColorEditFlags_NoLabel);
					ImGui::EndPopup();
				}
				ImGui::Text("Thickness: %.1f", g.boxThickness);
				ImGui::SliderFloat("##BoxThickness", &g.boxThickness, 1.0f, 5.0f); // Mudado para float
			}
			...
			if (g.healthBar)
			{
				...
				ImGui::SameLine();
				if (ImGui::ColorButton("##HealthBarColor", g.healthBarColor))
					ImGui::OpenPopup("Select HealthBar Color");
				if (ImGui::BeginPopup("Select HealthBar Color"))
				{
					ImGui::ColorEdit4("HealthBar Color", (float*)&g.healthBarColor, ImGuiColorEditFlags_NoInputs | ImGuiColorEditFlags_NoLabel);
					ImGui::EndPopup();
				}
				ImGui::Text("Thickness: %.1f", g.healthBarThickness);
				ImGui::SliderFloat("##HealthBarThickness", &g.healthBarThickness, 1.0f, 5.0f); // Mudado para float
			}
			...
			if (g.lines)
			{
				...
				ImGui::SameLine();
				if (ImGui::ColorButton("##LinesColor", g.linesColor))
					ImGui::OpenPopup("Select Lines Color");
				if (ImGui::BeginPopup("Select Lines Color"))
				{
					ImGui::ColorEdit4("Lines Color", (float*)&g.linesColor, ImGuiColorEditFlags_NoInputs | ImGuiColorEditFlags_NoLabel);
					ImGui::EndPopup();
				}
				ImGui::Text("Thickness: %.1f", g.lineThickness);
				ImGui::SliderFloat("##LineThickness", &g.lineThickness, 1.0f, 5.0f); // Mudado para float
			}
			...
			if (g.skeleton)
			{
				...
				ImGui::SameLine();
				if (ImGui::ColorButton("##SkeletonColor", g.skeletonColor))
					ImGui::OpenPopup("Select Skeleton Color");
				if (ImGui::BeginPopup("Select Skeleton Color"))
				{
					ImGui::ColorEdit4("Skeleton Color", (float*)&g.skeletonColor, ImGuiColorEditFlags_NoInputs | ImGuiColorEditFlags_NoLabel);
					ImGui::EndPopup();
				}
				ImGui::Text("Thickness: %.1f", g.skeletonThickness);
				ImGui::SliderFloat("##SkeletonThickness", &g.skeletonThickness, 1.0f, 3.0f); // Mudado para float
			}
		...
		{
			ImGui::Checkbox("no flash", &g.noflash);
			ImGui::EndTabItem();
		}
